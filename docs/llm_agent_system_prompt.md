# LLM 智能体系统提示（System Prompt）

> **用途**：将此内容配置到 Coze / Kimi 等 LLM 智能体的 **System Prompt** 中。  
> **效果**：业务代码每次调用时只需传入「模块类型 + 股票代码 + 已知事实」，可节约大量 Token。
...
业务代码每次调用 LLM 时，只需发送以下内容：

```
模块：moat / business_model / management
股票代码：600519
已知财务事实：
- 近5年毛利率数据：[91.5, 91.3, 91.9, 92.1, 92.5]%，标准差 0.45%，趋势：明显上升
```

LLM 根据 System Prompt 中预设的评分标准，结合已知事实和自身知识，直接输出对应模块的 JSON。
