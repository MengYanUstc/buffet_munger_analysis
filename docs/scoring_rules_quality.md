...
| 资产负债率 | 定量 | 2 | SQLite `financial_reports.debt_ratio` |
...
基于近4年 ROE 数据的**变异系数（CV = 标准差 / |均值|）**和趋势：

> 相比绝对标准差，CV 消除了不同 ROE 水平带来的量纲差异，更能反映真实的相对波动性。
...
**数据流**：`SQLite -> analyze_debt_ratio() -> 纯定量计算最终分`
