    ...
    def _check_cache(
        ...
        try:
            ...
            if row and row["response_json"]:
                ...
                # 若缓存全是 fallback（LLM 失败），视为未命中，重新调用
                if cached and all(
                    isinstance(v, dict) and ("LLM调用失败" in (v.get("reason") or "") or "回退到规则" in (v.get("reason") or ""))
            ...
            try:
                self.db.execute(
                    ...
                    (
                        ...
                        getattr(self.llm, "model", getattr(self.llm, "api_token", "")[:20] + "...") if self.llm else None,
                    ),
                )
            except Exception as e:
